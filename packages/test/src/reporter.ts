...
export class TestReporter {
    ...
    listen(job: TestJob) {
        job.connect("message", (job: TestJob, data: string) =>
            this.parseMessage(job.path, data)
        );
    }
    ...
    parseMessage(file: Gio.File, data: string): void {
        ...
        switch (message.type as keyof MessageType) {
            ...
            default:
                throw new Error(
                    `Unexpected message type: ${inspect(message.type)}`
                );
        }
    }
    ...
    onImportError(file: Gio.File, message: Message<"importError">) {
        Display.outln(
            `${c.redBright.bold(
                "ERROR:"
            )} Exception while loading file ${c.bold(this.mapPath(file))}:`
        );
    ...
    mapPath(file: Gio.File): string {
        ...
        const path = cwdiseFilename(
            this.sourceMaps.sourceFileFor(file.get_uri())
        );
    ...
    printTestResult(file: Gio.File, name: string, result: TestResult) {
        ...
        Display.outln(
            `${this.testDisplayName(file, name)}: ${theme[result](
                result.toUpperCase()
            )}`
        );
    }
    ...
    summarise() {
        ...
        if (this.timeout.length > 0) {
            ...
            for (const [file, name, timeout] of this.timeout) {
                Display.outln(
                    `  ${this.testDisplayName(file, name)}: ${timeout} ms`
                );
            }
            Display.outln();
        }
        ...
        for (const [file, name, err] of this.failed) {
            Display.outln(
                c.bold(
                    `${c.red("FAILED:")} ${this.testDisplayName(file, name)}`
                )
            );
...
function inlineDiff(actual: string, expected: string, theme: Theme): string {
    ...
    // legend
    msg = `\n    ${theme.diffRemoved("actual")} ${theme.diffAdded(
        "expected"
    )}\n\n${msg.replace(/^/gm, "    ")}\n`;
...
function errorDiff(actual: string, expected: string, theme: Theme): string {
    const diffed =
        actual.startsWith("{") ||
        (actual.startsWith("[") && actual.includes("\n"))
...
function _unifiedDiff(actual: string, expected: string, theme: Theme): string {
    ...
    const lines = msg.split("\n").splice(4);
    return `\n${indent}${theme.diffAdded("+ expected")} ${theme.diffRemoved(
        "- actual"
