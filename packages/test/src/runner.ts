import GLib from "@gx/gi/Gjs/GLib-2.0";
import Gio from "@gx/gi/Gjs/Gio-2.0";
import Gtk from "@gx/gi/Gjs/Gtk-4.0";
import { System } from "@gx/gi/Gjs/Gjs";
...
const options = /* @__PURE__ */ {
    ...
    timeout: Number.parseInt(
        GLib.getenv("GXTEST_RUNNER_TIMEOUT") ?? "3000",
        10
    ),
};

function deduceTestResult(
    test: Test,
...
async function runTests(
    app: Gtk.Application,
    exports: Promise<{ [key: string]: unknown }>
) {
    app.hold();
        ...
        send({
            ...
            error: errorDesc(
                err instanceof Error
                    ? err
                    : new Error(`Unknown error: ${inspect(err)}`)
            ),
        });
        app.quit();
        return;
    ...
    const runTest = async (
        ...
        const timeout =
            test.timeout !== undefined ? test.timeout : options.timeout;
        const threw = await Async.timeout(
            runAsPromise(test, cancel),
            timeout,
            cancel
        ).catch((err: Error) => err);
...
export function runner(exports: Promise<{ [key: string]: unknown }>) {
    ...
    app.connect("activate", () => {
        runTests(app, exports).catch((err) =>
            console.error("Toplevel exception:", err)
        );
    });
