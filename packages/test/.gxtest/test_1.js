...
__export(jsx_exports, {
  ...
  MoveType: () => MoveType,
  addChild: () => addChild,
  applyAccessible: () => applyAccessible,
  applyStyleClasses: () => applyStyleClasses,
  builder: () => builder,
  construct: () => construct,
  diff: () => diff,
  getElementById: () => getElementById,
  jsxGtkMeta: () => jsxGtkMeta,
...
function withInternal(obj) {
  let internal = obj[jsxInternal];
  if (internal === void 0) {
    internal = new InternalState();
    obj[jsxInternal] = internal;
  }
  const name = gtypeForObject(obj).name;
  obj.$uuid = `${name}#${internal.uuid}`;
  return obj;
}
function getElementById(id) {
  return builder.get_object(id) || void 0;
}
function construct(constructor, staticProps, cellProps) {
  if (isConstructor(GObject.Object, constructor)) {
    const props2 = { ...staticProps };
    let obj;
    for (const [prop2, effect2] of Object.entries(cellProps)) {
      props2[prop2] = effect(
        () => {
          const result = effect2();
          const oldValue = obj[prop2];
          if (!Object.is(oldValue, result)) {
            obj.set_property(prop2, result);
          }
          return result;
        },
        {
          name: `${constructor.$gtype.name}.${prop2}`,
          initial: () => effect2()
        }
      );
    }
    obj = new constructor(props2);
    console.log("Constructed:", logId(obj));
    return obj;
  }
  const props = object({ ...staticProps }, { name: constructor.name });
  for (const [prop2, effect2] of Object.entries(cellProps)) {
    props[prop2] = effect(
      () => {
        const result = effect2();
...
function applyStyleClasses(obj, styleClasses) {
  const widget = withInternal(assertCast(Gtk.Widget, obj));
  const current = new Set(widget.get_css_classes());
  if (widget[jsxInternal].cssDefaults === void 0) {
  ...
  for (const c2 of target) {
    if (!current.delete(c2)) {
      widget.add_css_class(c2);
    }
  }
  for (const c2 of current) {
    if (!defaults.has(c2)) {
      widget.remove_css_class(c2);
    }
  }
}
function applyAccessible(obj, desc) {
  const widget = assertCast(Gtk.Accessible, obj);
  if (desc.state !== void 0) {
    for (const [state, value] of desc.state) {
      if (isCell(value)) {
        effect(() => widget.update_state([state], [value.get()]));
      } else {
        widget.update_state([state], [value]);
      }
    }
  }
  if (desc.props !== void 0) {
    for (const [prop2, value] of desc.props) {
      if (isCell(value)) {
        effect(() => widget.update_property([prop2], [value.get()]));
      } else {
        widget.update_property([prop2], [value]);
      }
    }
  }
  if (desc.rel !== void 0) {
    for (const [rel, value] of desc.rel) {
      if (isCell(value)) {
        effect(() => widget.update_relation([rel], [value.get()]));
      } else {
...
function diff(oldList, newList) {
  ...
  function insert(index, item) {
    moves.push({ type: 1 /* Insert */, after: newList[index - 1] ?? null, item });
  }
  oldList.forEach((item) => {
    if (newIndex.has(item)) {
      simulateList.push(item);
    } else {
      moves.push({ type: 0 /* Remove */, item });
    }
  });
...
function logId(obj) {
  const name = gtypeForObject(obj).name;
  const uuid2 = withInternal(obj)[jsxInternal].uuid;
  return `${name}#${uuid2}`;
}
function syncChildren(parentWidget, children) {
  const parent = withInternal(parentWidget);
  const newChildren = Array.from(children);
  let oldChildren = parent[jsxInternal].children;
  if (oldChildren === void 0) {
    oldChildren = [];
    parent[jsxInternal].startChild = parent.get_last_child();
  }
  const startChild = parent[jsxInternal].startChild;
...
function addChild(parent, child) {
  const childMeta = child[jsxGtkMeta] ?? {};
  console.log("Adding child:", logId(child));
  parent.vfunc_add_child(builder, child, childMeta["type"] ?? null);
  if (childMeta.layout !== void 0) {
    const parentWidget = assertCast(Gtk.Widget, parent);
    const childWidget = assertCast(Gtk.Widget, child);
...
function removeChild(parent, child) {
  ...
  if (typeof parent.remove === "function") {
    parent.remove(child);
  } else if (isA(Gtk.Widget, child)) {
    child.unparent();
  } else {
    const childType = gtypeForObject(child).name;
    const parentType = gtypeForObject(parent).name;
    throw new Error(
      `Can't figure out how to remove child of type ${childType} from parent of type ${parentType}`
    );
  }
}
var jsxGtkMeta, jsxInternal, builder, uuid, InternalState, MoveType;
var init_jsx = __esm({
  "../core/jsx.ts"() {
    "use strict";
    init_gi();
    init_effect();
    init_gobject();
    jsxGtkMeta = /* @__PURE__ */ Symbol("JSX.Gtk.Meta");
    jsxInternal = /* @__PURE__ */ Symbol("JSX.Gtk.Internal");
    builder = /* @__PURE__ */ new Gtk.Builder();
    uuid = 1;
    InternalState = class {
      uuid = uuid++;
      cssDefaults;
    ...
    MoveType = /* @__PURE__ */ ((MoveType2) => {
      MoveType2[MoveType2["Remove"] = 0] = "Remove";
      MoveType2[MoveType2["Insert"] = 1] = "Insert";
      return MoveType2;
    })(MoveType || {});
  }
});
...
// test/jsx.test.ts
var jsx_test_exports = {};
__export(jsx_test_exports, {
  child_diff: () => child_diff
});
function applyDiff(list, diff2) {
  const result = list.slice();
  for (const move of diff2) {
    switch (move.type) {
      case jsx_exports.MoveType.Remove:
        result.splice(result.indexOf(move.item), 1);
        break;
      case jsx_exports.MoveType.Insert: {
...
function child_diff() {
  ...
  const diff2 = jsx_exports.diff(a, b);
  expect(diff2).deep.equals([
    { type: jsx_exports.MoveType.Remove, item: 1 },
    { type: jsx_exports.MoveType.Insert, after: null, item: 6 },
    { type: jsx_exports.MoveType.Insert, after: 5, item: 4 },
    { type: jsx_exports.MoveType.Insert, after: 4, item: 9 },
    { type: jsx_exports.MoveType.Insert, after: 8, item: 2 },
    { type: jsx_exports.MoveType.Insert, after: 2, item: 10 },
    { type: jsx_exports.MoveType.Insert, after: 10, item: 11 }
  ]);
  const result = applyDiff(a, diff2);
  expect(result).deep.equals(b);
}
var init_jsx_test = __esm({
  "test/jsx.test.ts"() {
    "use strict";
    init_core();
    init_test();
...
// .gxtest/test_1.harness.js
runner(Promise.resolve().then(() => (init_jsx_test(), jsx_test_exports)));
/*!
 * ### ._obj
 *
