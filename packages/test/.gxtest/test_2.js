...
var require_test = __commonJS({
  "../../node_modules/.pnpm/chai@4.3.6/node_modules/chai/lib/chai/utils/test.js"(exports, module) {
    ...
    module.exports = function test2(obj, args) {
...
// index.ts
function makeTest(testOrOptionsOrLabel, testOrOptions, test2) {
  let label = void 0;
  let options2 = void 0;
  if (typeof testOrOptionsOrLabel === "string") {
    ...
    if (typeof testOrOptions === "function") {
      ...
      options2 = testOrOptions;
    }
  } else if (typeof testOrOptionsOrLabel === "function") {
    test2 = testOrOptionsOrLabel;
  } else {
    options2 = testOrOptionsOrLabel;
    test2 = testOrOptions;
  }
  if (options2 === void 0) {
    options2 = {};
  ...
  if (label !== void 0) {
    options2.label = label;
  }
  if (test2 === void 0) {
    throw new Error("No test function specified!");
  }
  return Object.assign(test2, options2);
}
var test;
var init_test = __esm({
  "index.ts"() {
...
// test/test.test.ts
var test_test_exports = {};
__export(test_test_exports, {
  are_things_deep_equal: () => are_things_deep_equal,
  are_things_equal: () => are_things_equal,
  are_things_instances_of_things: () => are_things_instances_of_things,
  are_things_unequal: () => are_things_unequal,
  can_we_catch_a_throw: () => can_we_catch_a_throw,
  can_we_expect_timeouts: () => can_we_expect_timeouts,
  do_gobjects_implement_ginterfaces: () => do_gobjects_implement_ginterfaces
});
function are_things_unequal() {
  expect(1337).not.equals(1338);
}
function are_things_equal() {
  expect(1337).equals(1337);
}
function are_things_deep_equal() {
...
function are_things_instances_of_things() {
  class Animal {
  }
...
function do_gobjects_implement_ginterfaces() {
  const button = new Gtk.Button();
  const object2 = new GObject.Object();
  expect(button).implements(Gtk.Widget);
  expect(object2).not.implements(Gtk.Widget);
}
var can_we_expect_timeouts, can_we_catch_a_throw;
var init_test_test = __esm({
  "test/test.test.ts"() {
    "use strict";
    init_gi();
    init_core();
    init_test();
    can_we_expect_timeouts = test({ timeout: 100, throws: async_exports.TimeoutError }, () => {
      return new Promise((resolve) => {
        setTimeout(() => resolve(), 1e4);
      });
    });
    can_we_catch_a_throw = test({ throws: Error }, () => {
...
function deduceTestResult(test2, threw) {
  let success = false;
  if (threw instanceof async_exports.TimeoutError) {
    if (test2.throws !== void 0 && threw instanceof test2.throws) {
      return ["ok", threw];
    }
    return ["timeout", threw];
  }
  if (test2.throws !== void 0) {
    if (threw !== void 0 && threw instanceof test2.throws) {
      success = true;
    } else {
      threw = new DidNotThrowError(test2.throws, threw);
    }
...
async function runTests(app, exports) {
  ...
  const runTest = async (name, test2, cancel) => {
    const timeout2 = test2.timeout !== void 0 ? test2.timeout : options.timeout;
    const threw = await async_exports.timeout(runAsPromise(test2, cancel), timeout2, cancel).catch(
      (err2) => err2
    );
    const [result, error] = deduceTestResult(test2, threw);
    switch (result) {
      case "ok":
        send({ type: "ok", name });
        break;
  ...
  tests.forEach(([name, test2]) => {
    queue.add((cancel) => runTest(name, test2, cancel)).catch(() => {
    });
  });
...
// .gxtest/test_2.harness.js
runner(Promise.resolve().then(() => (init_test_test(), test_test_exports)));
/*!
 * ### ._obj
 *
